...
### 2.4 R↔Python 파라미터 차이 (검토 발견사항)

R 스크립트 작성 시 Python과 동일한 파라미터를 명시해야 결과가 일치한다. 기본값이 다른 경우를 아래에 정리한다.

**높은 위험 (기본값 불일치 → 결과 완전히 다름)**

| 메서드 | Python 기본값 | R 기본값 | R 스크립트에서 맞출 값 |
|--------|---------------|----------|----------------------|
| Chi-square 2x2 | Yates 보정 OFF (`correction=False`) | Yates 보정 ON (`correct=TRUE`) | `chisq.test(..., correct=FALSE)` |
| ANOVA (다원) | Type II SS (`typ=2`) | Type I SS (sequential) | `car::Anova(..., type=2)` |
| K-means 클러스터링 | `random_state` 지정 | `set.seed()` 별도 | seed 고정 + centroid 수렴값만 비교 |
| Stepwise 회귀 | p-value 기준 선택 | AIC 기준 (`step()`) | 최종 계수가 아닌 개별 통계량 비교, 또는 동일 모델 강제 |

**중간 위험 (알고리즘 차이 → 소수점 하위에서 차이)**

| 메서드 | 차이 원인 | 대응 |
|--------|-----------|------|
| Wilcoxon signed-rank | ties/zeros 처리 방식 차이 | ties-free 데이터 우선 사용, ties 데이터는 tolerance 0.001로 완화 |
| Levene 검정 | Python `center='median'` 지정, R 기본 `center='mean'` | `car::leveneTest(..., center='median')` |
| Mann-Whitney | 연속성 보정 차이 가능 | `wilcox.test(..., correct=FALSE)` 명시 |
| Dose-response | 최적화 알고리즘 차이 (LM vs R optimizer) | 수렴값의 상대오차 비교, tolerance 0.01 허용 |

**낮은 위험 (직접 대응, 차이 없음)**

t-test 전종, Pearson/Spearman 상관, 카이제곱 적합도, Fisher 정확, Shapiro-Wilk, ARIMA, KM 생존분석, Cox 회귀 등 대부분의 메서드.
...
기존 스크립트의 검증된 패턴 활용:
- `loadPyodide()` → `loadPackage(['numpy', 'scipy', 'statsmodels', 'scikit-learn'])` → Python 실행
- tolerance 비교 함수 (`isCloseEnough`)
- 컬러 콘솔 출력

기술 검증 결과:
- Worker Python 파일은 순수 함수 — Web Worker 프로토콜 의존 없음
- `pyodide.runPythonAsync()` 로 47개 메서드 독립 호출 가능
- `helpers.py` 모듈 등록 패턴 이미 구현됨 (pyodide-worker.ts)
...
검토 결과, 47개 메서드의 테스트 데이터 상태는 다음과 같다.

### 데이터 충분 (26개) — 즉시 검증 가능

Phase 1 전체 + 주요 Phase 2/3 메서드: t-test 4종, anova, two-way-anova, mann-whitney, wilcoxon, kruskal-wallis, correlation, partial-correlation, regression, logistic-regression, stepwise, chi-square-independence, pca, cluster, kaplan-meier, cox-regression, roc-curve, normality-test, reliability, repeated-measures-anova

기존 `test-data/`, `example-data/`에 검증급 CSV 존재.

### e2e만 존재 (18개) — 외부 데이터 소싱 필요

자동생성 테스트 데이터만 존재하여 검증 기준으로 부적합. R 내장 데이터셋 또는 교과서 데이터로 교체 필요.

friedman, sign-test, mcnemar, cochran-q, binomial-test, runs-test, ks-test, mood-median, poisson, ordinal-regression, dose-response, response-surface, chi-square-goodness, factor-analysis, discriminant, arima, seasonal-decompose, stationarity-test, manova, mixed-model, proportion-test

**소싱 전략**: R 내장 데이터셋 (`datasets::`, `MASS::`, `survival::`) 또는 통계 교과서 공개 데이터 활용. R 스크립트에서 데이터 + 기준값을 함께 생성.

### 완전 갭 (3개) — 데이터 신규 생성 필요

| 메서드 | 상황 | 소싱 계획 |
|--------|------|-----------|
| `mann-kendall` | 전용 CSV 없음 | 환경과학 시계열 데이터 (R `trend::mk.test` 예제) |
| `ancova` | 부적합 데이터만 존재 | R `datasets::ToothGrowth` 또는 교과서 데이터 |
| `welch-anova` | 전용 데이터 없음 | 분산 불균일 그룹 데이터 생성 (R `oneway.test` 예제) |
...
작업:
1. R 스크립트 작성 (`generate-references.R`) — 47개 메서드별 R 함수 호출 + JSON 출력
2. 섹션 2.4의 파라미터 차이 반영 — R 호출 시 Python 기본값과 일치하도록 명시
3. NIST StRD 데이터셋 다운로드 (Norris, Pontius, AtmWtAg 등)
4. 데이터 갭 18+3건 소싱 — R 내장 데이터셋 활용
