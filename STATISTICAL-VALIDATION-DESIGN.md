...
**Date**: 2026-04-06
**Updated**: 2026-04-06 (v7 — data tool 분리, power-analysis 재분류)
**Status**: Reviewed (v7)
**성격**: 기존 golden/Pyodide 검증 체계를 **독립 검증 체계로 승격**하는 마이그레이션 설계
**Purpose**: BioHub 46개 통계 메서드 + 4개 데이터 도구의 정확도를 R/NIST 기준으로 종합 검증하고, 증빙을 체계적으로 기록

> **ID 체계**: 본 문서의 모든 메서드 ID는 canonical ID (`statistical-methods.ts` SSOT 기준).
> 예: `two-sample-t` (~~t-test~~), `one-way-anova` (~~anova~~), `pearson-correlation` (~~correlation~~)
...
Layer 2: R 교차검증 (주력)
  대상: 46개 통계 메서드 (전체 동치성 계약) + 4개 데이터 도구 (간소화 검증)
  전제: 메서드별 동치성 계약 (부록 A) 에 따라 R 호출 옵션을 Python과 일치시킴
  방법: R 스크립트로 기준값 생성 → BioHub 결과와 비교
  우선순위: R > 교과서/논문값 > GraphPad 등 > SciPy 자체(최후 수단)
  데이터 도구 4개: descriptive-stats, explore-data, means-plot, power-analysis
    → 가설검정 통계량(p-value, test statistic) 없음
    → 산출값(평균, 분산, 검정력 등) 비교만 수행, LRE/Tier 2 기준 적용
...
|--------|------------------------|----------|------|
| KM 생존분석 | `worker5: kaplan_meier_analysis()` — **statsmodels SurvfuncRight + survdiff** | R `survival::survfit`과 직접 비교, LRE 15.0. edge case(전원 중도절단) 검증 완료 | tier2 |
...
t-test 4종 (`two-sample-t`, `welch-t`, `one-sample-t`, `paired-t`), `pearson-correlation` (Pearson/Spearman/Kendall 통합 엔트리), `chi-square-goodness`, `normality-test` 등. 이들은 scipy 함수를 단일 경로로 호출하며 R과 동일한 알고리즘을 사용한다.

### 2.5 실행 빈도

- **1회 종합 검증**: Pyodide 실제 로드, 46개 통계 + 4개 도구 전체 검증
...
- 검증 범위: 46개 통계 메서드 + 4개 데이터 도구
- 기준 소스: R 버전, NIST StRD 버전

## 검증 결과 요약
| 메서드 | Layer | 기준 소스 | 데이터셋 수 | PASS/FAIL | 평균 LRE |
|--------|-------|-----------|-------------|-----------|----------|
| one-way-anova | L1+L2 | NIST AtmWtAg + R 4.3.2 | 4 | PASS | 11.8 |
| simple-regression | L1+L2 | NIST Norris + R 4.3.2 | 3 | PASS | 13.2 |
| two-sample-t | L2 | R 4.3.2 | 5 | PASS | 12.3 |
...
## 검증 환경
- Pyodide 0.29.3, SciPy 1.14.1, statsmodels 0.14.1, sklearn 1.4.0, pingouin 0.5.4
...
```jsonc
{
  ...
  "summary": {
    "totalMethods": 50,  // 46 statistical + 4 data tools
    "totalCases": 215,
    "passed": 213,
    "failed": 2,
    "averageLRE": 11.2
...
# 특정 메서드만
pnpm test:validation -- --method two-sample-t
...
two-sample-t, welch-t, one-sample-t, paired-t, one-way-anova, two-way-anova, mann-whitney, wilcoxon-signed-rank, kruskal-wallis, pearson-correlation, simple-regression, logistic-regression, chi-square-independence, pca, cluster, kaplan-meier, cox-regression, roc-curve, normality-test, repeated-measures-anova, ancova

#### e2e만 존재 (24개) — 외부 데이터 소싱 필요

자동생성 테스트 데이터만 존재하여 검증 기준으로 부적합. R 내장 데이터셋 또는 교과서 데이터로 교체 필요:

friedman, sign-test, mcnemar, cochran-q, binomial-test, runs-test, kolmogorov-smirnov, mood-median, poisson-regression, ordinal-regression, dose-response, response-surface, chi-square-goodness, factor-analysis, discriminant-analysis, arima, seasonal-decompose, stationarity-test, manova, mixed-model, one-sample-proportion, reliability-analysis, stepwise-regression, partial-correlation

**소싱 전략**: R 내장 데이터셋 (`datasets::`, `MASS::`, `survival::`) 또는 통계 교과서 공개 데이터 활용. R 스크립트에서 데이터 + 기준값을 함께 생성.

#### 완전 갭 (1개) — 데이터 신규 생성 필요

| 메서드 | 상황 | 소싱 계획 |
|--------|------|-----------|
| `mann-kendall-test` | 전용 CSV 없음 | 환경과학 시계열 데이터 (R `trend::mk.test` 예제) |

### 6.2 데이터 도구 (4개) — 별도 검증 경로

데이터 도구는 가설검정 통계량을 산출하지 않으므로, 데이터셋 소싱이 아닌 **산출값/파라미터 fixture** 기반으로 검증한다.

| 메서드 | 검증 방식 | fixture 예시 |
|--------|-----------|-------------|
| `descriptive-stats` | 산출값 비교 (mean, sd, skew, kurtosis) | R `datasets::iris` 등 표준 데이터셋 |
| `explore-data` | 탐색 산출값 비교 (분포, 이상치 탐지) | 동일 데이터셋 |
| `means-plot` | 그룹 평균/CI 수치 비교 (시각화는 범위 밖) | 그룹화된 데이터셋 |
| `power-analysis` | 파라미터 fixture 비교 (R `pwr::pwr.t.test()` 등) | `{effectSize: 0.5, alpha: 0.05, n: 30}` → power 값 |
...
2. R 스크립트 작성 (`generate-references.R`) — 46개 통계 메서드 + 4개 도구 fixture
3. NIST StRD 데이터셋 다운로드 (Norris, Pontius, AtmWtAg 등)
4. 데이터 갭 24+1건 소싱 + 도구 4건 fixture 작성
...
- `runs-test`, `kolmogorov-smirnov`, `mood-median`
- `pearson-correlation`, `partial-correlation`
- `simple-regression`, `logistic-regression`, `poisson-regression`
- `ordinal-regression`, `stepwise-regression`, `dose-response`, `response-surface`

### Phase 3: 카이제곱 + 다변량 + 생존/시계열 + 기타
...
- `normality-test`, `reliability-analysis`, `one-sample-proportion`
- `repeated-measures-anova`, `ancova`, `manova`, `mixed-model`
- 데이터 도구: `descriptive-stats`, `explore-data`, `means-plot`, `power-analysis` (산출값 비교만)
...
| 기준 | 목표 |
|------|------|
| 통계 메서드 커버리지 | 46/46 (100%) |
| 데이터 도구 커버리지 | 4/4 (100%, 산출값 비교) |
| Layer 1 (NIST) | 해당 메서드 전수 PASS (ANOVA, 선형회귀, 기술통계) |
| Layer 2 (R) | 46개 통계 전수 PASS (Tier별 허용 오차) + 4개 도구 산출값 PASS |
...
- 새 통계 메서드 추가
- Playwright E2E 검증
- Bio-Tools 16개 도구 (통계 46개 + 데이터 도구 4개만 대상)
...
| 필드 | 설명 |
|------|------|
| `methodId` | BioHub canonical 메서드 ID (`statistical-methods.ts` SSOT 기준) |
...
|----------|-------|-----------|-------|---------------|-----------------|
| two-sample-t | L2 | ttest_ind(g1,g2, equal_var=True) | t.test(g1,g2, var.equal=TRUE) | tier2 | none |
...
| one-way-anova | L1+L2 | f_oneway(*groups) | aov(y~factor(g)) | tier2 | none |
...
전체 46개 통계 메서드 + 4개 도구의 Target Matrix는 Phase 0 작업 시 완성하여 별도 JSON 파일로 관리한다.
