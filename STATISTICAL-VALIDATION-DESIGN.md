...
**Date**: 2026-04-06
**Updated**: 2026-04-06 (v6 — canonical ID 반영)
**Status**: Reviewed (v6)
**성격**: 기존 golden/Pyodide 검증 체계를 **독립 검증 체계로 승격**하는 마이그레이션 설계
**Purpose**: BioHub 50개 통계 계산의 정확도를 R/NIST 기준으로 종합 검증하고, 증빙을 체계적으로 기록
...
- **1회 종합 검증**: Pyodide 실제 로드, 50개 메서드 전체 검증
...
| `explore-data` | 탐색 도구 — 기술통계 검증용 데이터 필요 | R `datasets::iris` 또는 교과서 데이터 |
...
2. R 스크립트 작성 (`generate-references.R`) — 50개 메서드별 R 함수 호출 + JSON 출력
3. NIST StRD 데이터셋 다운로드 (Norris, Pontius, AtmWtAg 등)
4. 데이터 갭 27+2건 소싱 — R 내장 데이터셋 활용
...
| Layer 2 (R) | 50개 전수 PASS (Tier별 허용 오차 적용) |
...
전체 50개 메서드의 Target Matrix는 Phase 0 작업 시 완성하여 별도 JSON 파일로 관리한다.
