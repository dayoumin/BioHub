...
기존 러너(`run-pyodide-golden-tests.mjs:65`)가 이미 절대/상대 오차를 함께 지원하므로, 이 구조를 그대로 사용한다.
...
| 메서드 | 차이 원인 | 대응 |
|--------|-----------|------|
| Wilcoxon signed-rank | ties/zeros 처리 방식 차이 | ties-free 데이터 우선 사용, ties 데이터는 Tier 3 적용 |
...
| Dose-response | 최적화 알고리즘 차이 (LM vs R optimizer) | 수렴값의 상대오차 비교, Tier 4 적용 |
