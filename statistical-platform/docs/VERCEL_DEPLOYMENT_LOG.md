...
├── statistical-platform/      ← Next.js 앱 (여기서 빌드)
│   ├── app/
│   ├── package.json
...
- `buildCommand`로 직접 지정: `cd statistical-platform && npm run build`
- `outputDirectory`는 루트 기준: `statistical-platform/out`
...
**해결**: vercel.json 확인
- Static Export: `outputDirectory: "statistical-platform/out"`
- Server Mode: `outputDirectory: "statistical-platform/.next"`
