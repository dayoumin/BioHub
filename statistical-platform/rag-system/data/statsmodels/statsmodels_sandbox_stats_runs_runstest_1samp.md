---
title: statsmodels.sandbox.stats.runs.runstest_1samp
description: Runs Test (단일표본)
source: https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_1samp.html
library: statsmodels
version: 0.14.4
license: BSD 3-Clause
copyright: (c) 2009-2024, statsmodels Developers
crawled_date: 2025-10-31
---

# statsmodels.sandbox.stats.runs.runstest_1samp

**Description**: Runs Test (단일표본)

**Original Documentation**: [statsmodels.sandbox.stats.runs.runstest_1samp](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_1samp.html)

---

[ Skip to content ](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_1samp.html#statsmodels.sandbox.stats.runs.runstest_1samp)
[ ![logo](https://www.statsmodels.org/stable/_static/statsmodels-logo-v2-bw.svg) ](https://www.statsmodels.org/stable/index.html "statsmodels 0.14.4")
statsmodels 0.14.4 
Stable
  * [Stable](https://www.statsmodels.org/stable/)
  * [Development](https://www.statsmodels.org/devel/)
  * [0.13](https://www.statsmodels.org/v0.13.5/)
  * [0.12](https://www.statsmodels.org/v0.12.2/)
  * [0.11](https://www.statsmodels.org/v0.11.1/)
  * [0.10](https://www.statsmodels.org/v0.10.2/)
  * [0.9](https://www.statsmodels.org/0.9.0/)
  * [0.8](https://www.statsmodels.org/0.8.0/)
  * [0.6](https://www.statsmodels.org/0.6.1/)


statsmodels.sandbox.stats.runs.runstest_1samp 
Type to start searching
[ statsmodels 
  * v0.14.5
  * 11k
  * 3.3k

](https://github.com/statsmodels/statsmodels/ "Go to repository")
[ ![logo](https://www.statsmodels.org/stable/_static/statsmodels-logo-v2-bw.svg) ](https://www.statsmodels.org/stable/index.html "statsmodels 0.14.4") statsmodels 0.14.4 
[ statsmodels 
  * v0.14.5
  * 11k
  * 3.3k

](https://github.com/statsmodels/statsmodels/ "Go to repository")
  * [ Installing statsmodels ](https://www.statsmodels.org/stable/install.html)
  * [ Getting started ](https://www.statsmodels.org/stable/gettingstarted.html)
  * [User Guide](https://www.statsmodels.org/stable/user-guide.html)
User Guide
    * [ Background ](https://www.statsmodels.org/stable/user-guide.html#background)
    * [ Regression and Linear Models ](https://www.statsmodels.org/stable/user-guide.html#regression-and-linear-models)
    * [ Time Series Analysis ](https://www.statsmodels.org/stable/user-guide.html#time-series-analysis)
    * [ Other Models ](https://www.statsmodels.org/stable/user-guide.html#other-models)
    * [Statistics and Tools](https://www.statsmodels.org/stable/user-guide.html#statistics-and-tools)
Statistics and Tools
      * [Statistics stats](https://www.statsmodels.org/stable/stats.html)
Statistics stats
        * [ Residual Diagnostics and Specification Tests ](https://www.statsmodels.org/stable/stats.html#module-statsmodels.stats.stattools)
        * [ Sandwich Robust Covariances ](https://www.statsmodels.org/stable/stats.html#sandwich-robust-covariances)
        * [ Goodness of Fit Tests and Measures ](https://www.statsmodels.org/stable/stats.html#goodness-of-fit-tests-and-measures)
        * [Non-Parametric Tests](https://www.statsmodels.org/stable/stats.html#module-statsmodels.sandbox.stats.runs)
Non-Parametric Tests
          * [ statsmodels.sandbox.stats.runs.mcnemar ](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.mcnemar.html)
          * [ statsmodels.sandbox.stats.runs.symmetry_bowker ](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.symmetry_bowker.html)
          * [ statsmodels.sandbox.stats.runs.median_test_ksample ](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.median_test_ksample.html)
          * statsmodels.sandbox.stats.runs.runstest_1samp [ statsmodels.sandbox.stats.runs.runstest_1samp ](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_1samp.html)
            * [ Fstatsmodels.sandbox.stats.runs.runstest_1samp ](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_1samp.html#statsmodels.sandbox.stats.runs.runstest_1samp)
              * [ Parameters ](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_1samp.html#statsmodels.sandbox.stats.runs.runstest_1samp-parameters)
              * [ Returns ](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_1samp.html#statsmodels.sandbox.stats.runs.runstest_1samp-returns)
          * [ statsmodels.sandbox.stats.runs.runstest_2samp ](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_2samp.html)
          * [ statsmodels.sandbox.stats.runs.cochrans_q ](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.cochrans_q.html)
          * [ statsmodels.sandbox.stats.runs.Runs ](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.Runs.html)
          * [ statsmodels.stats.descriptivestats.sign_test ](https://www.statsmodels.org/stable/generated/statsmodels.stats.descriptivestats.sign_test.html)
          * [ statsmodels.stats.nonparametric.rank_compare_2indep ](https://www.statsmodels.org/stable/generated/statsmodels.stats.nonparametric.rank_compare_2indep.html)
          * [ statsmodels.stats.nonparametric.rank_compare_2ordinal ](https://www.statsmodels.org/stable/generated/statsmodels.stats.nonparametric.rank_compare_2ordinal.html)
          * [ statsmodels.stats.nonparametric.RankCompareResult ](https://www.statsmodels.org/stable/generated/statsmodels.stats.nonparametric.RankCompareResult.html)
          * [ statsmodels.stats.nonparametric.cohensd2problarger ](https://www.statsmodels.org/stable/generated/statsmodels.stats.nonparametric.cohensd2problarger.html)
          * [ statsmodels.stats.nonparametric.prob_larger_continuous ](https://www.statsmodels.org/stable/generated/statsmodels.stats.nonparametric.prob_larger_continuous.html)
          * [ statsmodels.stats.nonparametric.rankdata_2samp ](https://www.statsmodels.org/stable/generated/statsmodels.stats.nonparametric.rankdata_2samp.html)
        * [ Descriptive Statistics ](https://www.statsmodels.org/stable/stats.html#module-statsmodels.stats.descriptivestats)
        * [ Interrater Reliability and Agreement ](https://www.statsmodels.org/stable/stats.html#interrater-reliability-and-agreement)
        * [ Multiple Tests and Multiple Comparison Procedures ](https://www.statsmodels.org/stable/stats.html#multiple-tests-and-multiple-comparison-procedures)
        * [ Basic Statistics and t-Tests with frequency weights ](https://www.statsmodels.org/stable/stats.html#basic-statistics-and-t-tests-with-frequency-weights)
        * [ Power and Sample Size Calculations ](https://www.statsmodels.org/stable/stats.html#power-and-sample-size-calculations)
        * [ Proportion ](https://www.statsmodels.org/stable/stats.html#proportion)
        * [ Rates ](https://www.statsmodels.org/stable/stats.html#rates)
        * [ Multivariate ](https://www.statsmodels.org/stable/stats.html#multivariate)
        * [ Oneway Anova ](https://www.statsmodels.org/stable/stats.html#oneway-anova)
        * [ Robust, Trimmed Statistics ](https://www.statsmodels.org/stable/stats.html#robust-trimmed-statistics)
        * [ Moment Helpers ](https://www.statsmodels.org/stable/stats.html#moment-helpers)
        * [ Mediation Analysis ](https://www.statsmodels.org/stable/stats.html#mediation-analysis)
        * [ Oaxaca-Blinder Decomposition ](https://www.statsmodels.org/stable/stats.html#oaxaca-blinder-decomposition)
        * [ Distance Dependence Measures ](https://www.statsmodels.org/stable/stats.html#distance-dependence-measures)
        * [ Meta-Analysis ](https://www.statsmodels.org/stable/stats.html#meta-analysis)
      * [ Contingency tables ](https://www.statsmodels.org/stable/contingency_tables.html)
      * [ Multiple Imputation with Chained Equations ](https://www.statsmodels.org/stable/imputation.html)
      * [ Treatment Effects treatment ](https://www.statsmodels.org/stable/treatment.html)
      * [ Empirical Likelihood emplike ](https://www.statsmodels.org/stable/emplike.html)
      * [ Distributions ](https://www.statsmodels.org/stable/distributions.html)
      * [ Graphics ](https://www.statsmodels.org/stable/graphics.html)
      * [ Input-Output iolib ](https://www.statsmodels.org/stable/iolib.html)
      * [ Tools ](https://www.statsmodels.org/stable/tools.html)
      * [ Working with Large Data Sets ](https://www.statsmodels.org/stable/large_data.html)
      * [ Optimization ](https://www.statsmodels.org/stable/optimization.html)
    * [ Data Sets ](https://www.statsmodels.org/stable/user-guide.html#data-sets)
    * [ Sandbox ](https://www.statsmodels.org/stable/user-guide.html#sandbox)
  * [ Examples ](https://www.statsmodels.org/stable/examples/index.html)
  * [ API Reference ](https://www.statsmodels.org/stable/api.html)
  * [ About statsmodels ](https://www.statsmodels.org/stable/about.html)
  * [ Developer Page ](https://www.statsmodels.org/stable/dev/index.html)
  * [ Release Notes ](https://www.statsmodels.org/stable/release/index.html)


  * [ Fstatsmodels.sandbox.stats.runs.runstest_1samp ](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_1samp.html#statsmodels.sandbox.stats.runs.runstest_1samp)
    * [ Parameters ](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_1samp.html#statsmodels.sandbox.stats.runs.runstest_1samp-parameters)
    * [ Returns ](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_1samp.html#statsmodels.sandbox.stats.runs.runstest_1samp-returns)


# statsmodels.sandbox.stats.runs.runstest_1samp[¶](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_1samp.html#statsmodels-sandbox-stats-runs-runstest-1samp "Link to this heading") 

statsmodels.sandbox.stats.runs.runstest_1samp(_[x](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_1samp.html#statsmodels.sandbox.stats.runs.runstest_1samp "statsmodels.sandbox.stats.runs.runstest_1samp.x \(Python parameter\)")_ , _[cutoff](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_1samp.html#statsmodels.sandbox.stats.runs.runstest_1samp "statsmodels.sandbox.stats.runs.runstest_1samp.cutoff \(Python parameter\)") =`'mean'`_, _[correction](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_1samp.html#statsmodels.sandbox.stats.runs.runstest_1samp "statsmodels.sandbox.stats.runs.runstest_1samp.correction \(Python parameter\)") =`True`_)[[source]](https://www.statsmodels.org/stable/_modules/statsmodels/sandbox/stats/runs.html#runstest_1samp)[¶](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_1samp.html#statsmodels.sandbox.stats.runs.runstest_1samp "Link to this definition") 
    
use runs test on binary discretized data above/below cutoff 

Parameters:[¶](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_1samp.html#statsmodels.sandbox.stats.runs.runstest_1samp-parameters "Permalink to this headline") 
     

**x**[ array_like](https://numpy.org/doc/stable/glossary.html#term-array_like "\(in NumPy v2.1\)") 
    
data, numeric 

**cutoff**{‘mean’, ‘median’} `or` `number` 
    
This specifies the cutoff to split the data into large and small values. 

**correction**[ bool](https://docs.python.org/3/library/stdtypes.html#bltin-boolean-values "\(in Python v3.12\)") 
    
Following the SAS manual, for samplesize below 50, the test statistic is corrected by 0.5. This can be turned off with correction=False, and was included to match R, tseries, which does not use any correction. 

Returns:[¶](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_1samp.html#statsmodels.sandbox.stats.runs.runstest_1samp-returns "Permalink to this headline") 
     

**z_stat**[`float`](https://docs.python.org/3/library/functions.html#float "\(in Python v3.12\)") 
    
test statistic, asymptotically normally distributed 

**p-value**[`float`](https://docs.python.org/3/library/functions.html#float "\(in Python v3.12\)") 
    
p-value, reject the null hypothesis if it is below an type 1 error level, alpha .
* * *
Last update: Oct 03, 2024 
[ Previous  statsmodels.sandbox.stats.runs.median_test_ksample  ](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.median_test_ksample.html) [ Next  statsmodels.sandbox.stats.runs.runstest_2samp  ](https://www.statsmodels.org/stable/generated/statsmodels.sandbox.stats.runs.runstest_2samp.html)
© Copyright 2009-2023, Josef Perktold, Skipper Seabold, Jonathan Taylor, statsmodels-developers. 
Created using [Sphinx](https://www.sphinx-doc.org/) 7.3.7. and [Sphinx-Immaterial](https://github.com/jbms/sphinx-immaterial/)
[ ](https://github.com/statsmodels/statsmodels/ "Source on github.com") [ ](https://pypi.org/project/statsmodels/ "pypi.org") [ ](https://doi.org/10.5281/zenodo.593847 "doi.org")
