/**
 * Batch 3-10: poisson 페이지 마이그레이션 검증 테스트
 *
 * 검증 항목:
...
const PAGE_PATH = join(process.cwd(), 'app/(dashboard)/statistics/poisson/page.tsx')

describe('Batch 3-10: poisson 페이지 마이그레이션 검증', () => {
  ...
  describe('7. 변수 선택 검증', () => {
    it('dependent + independent 변수 패턴이 있어야 함', () => {
      expect(fileContent).toMatch(/dependent: ''/)
      expect(fileContent).toMatch(/independent: \[\]/)
    })

    it('Alert 컴포넌트로 변수 선택 안내가 있어야 함', () => {
      expect(fileContent).toMatch(/종속변수 1개\(카운트 데이터\) \+ 독립변수 1개 이상 선택/)
    })
    ...
    it('종속변수와 독립변수 선택이 분리되어 있어야 함', () => {
      expect(fileContent).toMatch(/종속변수 \(카운트 데이터\)/)
      expect(fileContent).toMatch(/독립변수 \(1개 이상 선택\)/)
    })
  })

  describe('8. 백업 파일', () => {
    it('page.tsx.backup 파일이 존재해야 함', () => {
      const backupPath = join(process.cwd(), 'app/(dashboard)/statistics/poisson/page.tsx.backup')
      expect(() => readFileSync(backupPath, 'utf-8')).not.toThrow()
    })
  })

  describe('9. TypeScript 타입 안전성', () => {
    it('PoissonVariables 타입을 import 해야 함', () => {
      expect(fileContent).toMatch(/import type \{ PoissonVariables \}/)
    })

    it('PoissonRegressionResult 인터페이스가 있어야 함', () => {
      expect(fileContent).toMatch(/interface PoissonRegressionResult/)
    })

    it('useStatisticsPage에 타입 파라미터를 전달해야 함', () => {
      expect(fileContent).toMatch(/useStatisticsPage<PoissonRegressionResult, PoissonVariables>/)
    })
  ...
  describe('10. PyodideCore 통합', () => {
    ...
    it('Worker 2를 호출해야 함 (hypothesis)', () => {
      expect(fileContent).toMatch(/callWorkerMethod[\s\S]{0,200}2,?\s*\/\/ worker2-hypothesis/)
    })

    it('poisson_regression 메서드를 호출해야 함', () => {
      expect(fileContent).toMatch(/['"]poisson_regression['"]/)
    })
  })

  describe('11. Poisson Regression 특화 기능', () => {
    ...
    it('6개 탭이 있어야 함 (개요, 계수, 가정검정, 예측, 발생률비, 해석)', () => {
      expect(fileContent).toMatch(/overview/)
      expect(fileContent).toMatch(/coefficients/)
      expect(fileContent).toMatch(/assumptions/)
      expect(fileContent).toMatch(/predictions/)
      expect(fileContent).toMatch(/ratios/)
      expect(fileContent).toMatch(/interpretation/)
    })

    it('Table 컴포넌트를 사용해야 함', () => {
      expect(fileContent).toMatch(/Table/)
    })

    it('Recharts 차트가 있어야 함 (BarChart, ScatterChart)', () => {
      expect(fileContent).toMatch(/BarChart/)
      expect(fileContent).toMatch(/ScatterChart/)
    })

    it('PValueBadge를 사용해야 함', () => {
      expect(fileContent).toMatch(/PValueBadge/)
    })

    it('과산포(Overdispersion) 검정이 있어야 함', () => {
      expect(fileContent).toMatch(/과산포 검정/)
      expect(fileContent).toMatch(/dispersion_ratio/)
    })

    it('발생률비(IRR) 해석이 있어야 함', () => {
      expect(fileContent).toMatch(/발생률비/)
      expect(fileContent).toMatch(/IRR/)
    })
  })
})

describe('Batch 3-10: 코드 품질 체크', () => {
  let fileContent: string
