/**
 * Batch 3-8: proportion-test 페이지 마이그레이션 검증 테스트
 *
 * 검증 항목:
...
const PAGE_PATH = join(process.cwd(), 'app/(dashboard)/statistics/proportion-test/page.tsx')

describe('Batch 3-8: proportion-test 페이지 마이그레이션 검증', () => {
  ...
  describe('7. 변수 선택 검증', () => {
    ...
    it('Alert 컴포넌트로 변수 선택 안내가 있어야 함', () => {
      expect(fileContent).toMatch(/이분 변수 1개를 선택해주세요/)
    })
    ...
    it('분석 옵션 (testProportion, confidenceLevel, alternative, method)이 있어야 함', () => {
      expect(fileContent).toMatch(/testProportion/)
      expect(fileContent).toMatch(/confidenceLevel/)
      expect(fileContent).toMatch(/alternative/)
      expect(fileContent).toMatch(/method/)
    })
  })

  describe('8. 백업 파일', () => {
    it('page.tsx.backup 파일이 존재해야 함', () => {
      const backupPath = join(process.cwd(), 'app/(dashboard)/statistics/proportion-test/page.tsx.backup')
      expect(() => readFileSync(backupPath, 'utf-8')).not.toThrow()
    })
  })

  describe('9. TypeScript 타입 안전성', () => {
    it('ProportionTestVariables 타입을 import 해야 함', () => {
      expect(fileContent).toMatch(/import type \{ ProportionTestVariables \}/)
    })

    it('ProportionTestResults 인터페이스가 있어야 함', () => {
      expect(fileContent).toMatch(/interface ProportionTestResults/)
    })
    ...
    it('useStatisticsPage에 타입 파라미터를 전달해야 함', () => {
      expect(fileContent).toMatch(/useStatisticsPage<ProportionTestResults, ProportionTestVariables>/)
    })
  ...
  describe('10. PyodideCore 통합', () => {
    ...
    it('Worker 1을 호출해야 함 (descriptive)', () => {
      expect(fileContent).toMatch(/callWorkerMethod[\s\S]{0,200}1,?[\s\S]{0,50}\/\/ worker1-descriptive/)
    })

    it('proportion_test 메서드를 호출해야 함', () => {
      expect(fileContent).toMatch(/['"]proportion_test['"]/)
    })
  })

  describe('11. Proportion Test 특화 기능', () => {
    ...
    it('4개 탭이 있어야 함 (요약, 검정결과, 신뢰구간, 방법설명)', () => {
      expect(fileContent).toMatch(/summary/)
      expect(fileContent).toMatch(/results/)
      expect(fileContent).toMatch(/confidence/)
    ...
    it('Wilson Score Interval 설명이 있어야 함', () => {
      expect(fileContent).toMatch(/Wilson Score/)
    })

    it('연속성 보정 (Continuity Correction) 설명이 있어야 함', () => {
      expect(fileContent).toMatch(/Continuity Correction/)
    })
