...
describe('Smart Flow Store - Hub & InputMode', () => {
  ...
  describe('purposeInputMode 기본 동작', () => {
    ...
    it('setPurposeInputMode로 "browse"로 변경할 수 있다', () => {
      act(() => {
        useAnalysisStore.getState().setPurposeInputMode('browse')
      })

      expect(useAnalysisStore.getState().purposeInputMode).toBe('browse')
    })

    it('setPurposeInputMode로 다시 "ai"로 변경할 수 있다', () => {
      act(() => {
        useAnalysisStore.getState().setPurposeInputMode('browse')
        useAnalysisStore.getState().setPurposeInputMode('ai')
      })

      expect(useAnalysisStore.getState().purposeInputMode).toBe('ai')
    })
  })
  ...
  describe('resetSession', () => {
    ...
    it('resetSession이 showHub를 true로 리셋한다', () => {
      ...
      expect(useAnalysisStore.getState().showHub).toBe(true)
    })

    it('resetSession이 quickAnalysisMode를 false로 리셋한다', () => {
      act(() => {
        useAnalysisStore.getState().setQuickAnalysisMode(true)
        useAnalysisStore.getState().resetSession()
      })

      expect(useAnalysisStore.getState().quickAnalysisMode).toBe(false)
    })

    it('resetSession이 isReanalysisMode를 false로 리셋한다', () => {
      act(() => {
        useAnalysisStore.getState().setIsReanalysisMode(true)
        useAnalysisStore.getState().resetSession()
      })

      expect(useAnalysisStore.getState().isReanalysisMode).toBe(false)
    })
  })
