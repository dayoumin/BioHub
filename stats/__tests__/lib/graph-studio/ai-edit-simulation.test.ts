...
describe('AI 편집 시뮬레이션', () => {
  ...
  it('S4. Y축 제목 → encoding.y.title = "Weight (kg)"', async () => {
    mockRaw.mockResolvedValue(mockResponse([
      { op: 'add', path: '/encoding/y/title', value: 'Weight (kg)' },
    ]));
  ...
  it('S7. 연속 2회 편집 — 누적 적용 검증', async () => {
    ...
    mockRaw.mockResolvedValueOnce(mockResponse([
      { op: 'add', path: '/encoding/y/title', value: 'Mass (g)' },
    ]));
    // 2차: 스타일 변경
    mockRaw.mockResolvedValueOnce(mockResponse([
  ...
  // ── S11: 존재하지 않는 부모 경로 → patch validation 실패 ──

  it('S11. 부모 경로 없는 patch → applyAndValidatePatches 실패', async () => {
    mockRaw.mockResolvedValue(mockResponse([
