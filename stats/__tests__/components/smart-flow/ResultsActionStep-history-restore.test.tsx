...
vi.mock('@/components/smart-flow/ResultsVisualization', () => ({ ResultsVisualization: () => null }))
vi.mock('@/components/smart-flow/steps/results/MethodSpecificResults', () => ({ MethodSpecificResults: () => null }))
...
function setupStore(results: AnalysisResult, historyId: string | null = null): void {
  ...
  if (historyId) {
    ...
    useSmartFlowStore.setState({ currentHistoryId: historyId })
  }
}
...
describe('3차 리뷰 이슈 1: 같은 세션 히스토리 전환 — cache miss 시 자동 해석', () => {
  beforeEach(async () => {
    ...
    // Lazy import
    const mod = await import('@/components/smart-flow/steps/ResultsActionStep')
