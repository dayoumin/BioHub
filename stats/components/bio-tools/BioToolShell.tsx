...
export function BioToolShell({ tool, children, className }: BioToolShellProps): React.ReactElement {
  ...
  return (
    <div className={cn('flex flex-col h-full min-h-0', className)} style={BIO_BG_TINT}>
      ...
      >
        <div className={cn(LAYOUT.maxWidth, 'h-12 flex items-center gap-4', BIO_LAYOUT.contentPaddingX)}>
          <Link
            href="/bio-tools"
      ...
      <div className="flex-1 overflow-auto">
        <div className={cn(LAYOUT.maxWidth, BIO_LAYOUT.contentPaddingX, BIO_LAYOUT.contentPaddingY)}>
          {children}
        </div>
      </div>
    </div>
