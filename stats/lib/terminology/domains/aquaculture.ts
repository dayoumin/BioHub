...
export const aquaculture: TerminologyDictionary<'aquaculture'> = {
  ...
  analysis: {
    ...
    executionLogs: {
      ...
      assumptionSkippedWarning: '가정 검정이 실행되지 않았습니다. 데이터 조건이 맞지 않아 건너뛰었으며, 결과 해석 시 주의가 필요합니다.',
  ...
  hub: {
    ...
    quickMethodNames: {
      't-test': '두 집단 차이 비교 (독립표본)',
      'two-sample-t': '두 집단 차이 비교 (독립표본)',
      'anova': '세 집단 이상 비교',
      ...
      'wilcoxon-signed-rank': '이전·이후 비교 (비모수)',
      'kruskal-wallis': '세 집단 이상 비교 (비모수)',
      'fisher-exact': '소표본 연관성 검정',
    },
    recommendedAnalyses: {
      title: '추천 분석',
      startButton: '시작하기',
  ...
  dataExploration: {
    ...
    summaryCards: {
      ...
      variables: (n: number) => `${n} 변수`,
      outlierCount: (n: number) => `이상치 ${n}건`,
      noOutliers: '이상치 없음',
      normalitySummary: (pass: number, fail: number) => `정규성 ✓${pass} ✗${fail}`,
      normalityTesting: '정규성 검정 중...',
      homogeneityTesting: '등분산 검정 중...',
      homogeneityPass: '등분산 ✓',
      homogeneityFail: '등분산 ✗',
      maxCorrelation: (r: string) => `최대 r = ${r}`,
      strongPairs: (n: number) => `강한 상관 ${n}쌍`,
      needsTwoNumeric: '수치형 변수 2개 이상 필요',
    },
    features: {
    ...
    columnPanel: {
      ...
      categorical: '범주형',
      sampleSize: '표본 수',
      missingValuesLabel: '결측치',
      missingValues: (count: number) => `${count}건`,
      totalColumns: '전체 컬럼',
      recommendedAnalysis: '권장 분석',
      parametric: '모수적',
      nonParametric: '비모수적',
      columnList: '컬럼 목록',
      numericShort: '수치',
      categoricalShort: '범주',
      rowColCount: (rows: number, cols: number) => `${rows}행 × ${cols}열`,
      rowCount: (rows: number) => `${rows}행`,
    },
  ...
  results: {
    ...
    conclusion: {
      ...
      assumptionNotTested: '가정 검정이 실행되지 않았습니다. 데이터 조건이 맞지 않아 건너뛰었으며, 결과를 참고 수준으로 해석해 주세요.',
    ...
    metadata: {
      ...
      rValidated: 'R 검증 완료',
      customImplementationTooltip: '자체 구현 (Pyodide 라이브러리 제약)',
      computedWithLib: (lib: string) => `${lib} 기반 계산`,
      rCrossValidationLre: (lre: string) => `R 교차검증 정밀도 LRE ${lre} / 15.0`,
      optionsLabel: '옵션',
    },
    buttons: {
      saved: '저장됨',
    ...
    followUp: {
      ...
      waitingPlaceholder: 'AI 해석이 완료되면 후속 질문을 할 수 있습니다.',
    ...
    actionPanel: {
      title: '다음 작업',
      description: '현재 결과를 더 다듬을지, 별도의 새 분석 흐름을 시작할지 선택할 수 있습니다.',
      continueTitle: '현재 결과 이어서',
      continueDescription: '같은 데이터와 방법을 유지한 채 변수나 설정을 조정합니다.',
      restartTitle: '새 흐름 시작',
      restartDescription: '현재 결과와 분리해서 새 데이터나 새 분석 목적부터 다시 시작합니다.',
      toolsEyebrow: '보조 도구',
      toolsTitle: '추가 작업',
      toolsDescription: '방법 변경, 템플릿 저장, Graph Studio 이동',
      graphStudio: 'Graph Studio',
    },
    contextPanels: {
      historyViewTitle: '히스토리 결과 보기',
      historyViewDescription: '이전 단계 데이터는 저장되지 않아 다시 열 수 없습니다. 같은 방법으로 다시 확인하려면 재분석을 사용하세요.',
      documentsTitle: '이 결과를 사용하는 문서',
      documentsDescription: '자동 생성한 문서나 보고서를 수정한 뒤에도 원본 통계와 사용처를 바로 왕복할 수 있습니다.',
      moreDocuments: (count: number) => `+${count}개 더 있음`,
    },
  },
