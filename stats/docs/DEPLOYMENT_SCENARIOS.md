...
# 3. 실행 파일 위치
# Windows: src-tauri/target/release/stats.exe
...
# ☑️ 5. 실행 파일 테스트
# Windows: src-tauri/target/release/stats.exe 실행
...
> ⚠️ **중요사항**:
> - `outputDirectory`: `stats/out` (Static Export 모드 사용)
> - `framework`: `null` (Vercel 자동 감지 비활성화)
> - `npm ci` 대신 `npm install --legacy-peer-deps` 사용

> ⚠️ **중요**: `vercel.json`은 **저장소 루트**에 위치해야 합니다. `stats/vercel.json`은 무시될 수 있습니다.
...
1. **`vercel.json` 확인**:
   ```json
   {
     "outputDirectory": "stats/out",  // ✅ out (Static Export)
...
grep -A5 "overrides" stats/package.json

# ☑️ 4. Tailwind 버전 확인
grep "tailwindcss" stats/package.json
