...
|:--- |:--- |:--- |
| **Word Export** | ✅ Ready | Clean tables, APA style, AI text included. |
| **HTML Export** | ✅ Ready | Modern UI, responsive, print-friendly, dynamic text escaped. |
| **History Export** | ✅ Ready | Works for past analysis records. AI/APA is included when those fields exist in saved history payload. |
| **Charts/Graphs** | ❌ **Missing** | UI option exists but logic is not implemented. Charts are not exported. |
| **Raw Data** | ⚠️ Partial | Option works in `ResultsActionStep` (preview up to 200 rows). History export cannot include raw data by design. |
| **Methodology / References** | ⚠️ Partial | Option works in `ResultsActionStep` and `AnalysisHistoryPanel`. Other export entry points use defaults. |
