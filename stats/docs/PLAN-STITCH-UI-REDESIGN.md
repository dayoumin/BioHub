...
- ✅ AnalysisOptions 하드코딩 한글 5개 → terminology 이동 (`selectorUI.labels`)
- ✅ 사이드바 STITCH 디자인 언어 적용 (좌측 accent bar, pill 배지, 호버 투명도, 타이포 tracking)
- ✅ PurposeInputStep 간격 통일 (`mb-5` → `mb-6`, space-y-6 패턴)
- ✅ 테스트 mock 업데이트 (selectorUI.labels 추가)
- 사이드바 한글 하드코딩 유지 (네비게이션 라벨은 terminology 범위 밖)
- stepShortLabels: 이미 `SmartFlowText`에 존재, 추가 불필요

**파일**: `AnalysisOptions.tsx`, `VariableSelectionStep.tsx`, `PurposeInputStep.tsx`, `app-sidebar.tsx`, terminology 3파일, 테스트 1파일
**Step 계약 영향**: 없음
