...
> 기존 `analysis-e2e.spec.ts` 강화 + 누락 시나리오 보완
...
## 기존 테스트 현황 (analysis-e2e.spec.ts)
...
```
파일: analysis-e2e.spec.ts (기존 파일에 추가)
...
```bash
# Phase 1 전체
npx playwright test e2e/analysis-e2e.spec.ts --headed

# Smoke만
npx playwright test --grep "@smoke" e2e/analysis-e2e.spec.ts
```
