...
- [ ] **기존 문서 저널 profile 수정 UI 후속**: `DocumentEditor` 또는 문서 설정 패널에서 기존 문서의 target journal/style profile을 수정하고, profile version 변경 시 기존 preflight report가 stale로 표시되도록 한다. 저장 conflict/autosave와 얽히므로 별도 profile store 없이 metadata snapshot patch부터 시작한다.
