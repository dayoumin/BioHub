...
- [ ] **LLM review job store/진행 상태 후속**: 현재는 `DocumentEditor` run sequence guard로 late overwrite를 막는다. 다음 단계에서는 `DocumentReviewJobState` sidecar를 만들어 deterministic/LLM 단계별 `running | partial | completed | failed`, retry, timeout, late result discard, cross-tab 진행 상태 표시를 분리한다.
