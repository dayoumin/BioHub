...
- [ ] matplotlib export Worker 통합 — Pyodide Worker에 matplotlib 로드 + ChartSpec → matplotlib 변환기
- [ ] ExportDialog에 "논문용 Export" 옵션 추가 — PDF/TIFF/EPS + SciencePlots 스타일 선택
...
- [ ] Focus ring 표준 상수 — `card-styles.ts`에 `focusRing` 추가 (접근성 3건 의존)
- [ ] 토스트 메시지 상수 — `lib/constants/toast-messages.ts` (에러 관련 작업 의존)
- [ ] `useErrorRecovery` 훅 — retry 카운트 + 소진 판정 (AI 해석, 변수감지, 허브 에러 3건 의존)

---

## 디자인/UX 개선 사항 (2026-03-20 리뷰)

### 컴포넌트 재사용

- [ ] `paper-draft/PaperDraftPanel.tsx` (927줄) 정리 — 미사용 (demo만 참조). 삭제 또는 운영 코드와 통합
- [ ] 인라인 카드 패턴 통합 — `p-4 rounded-xl border` 10+곳 반복. 필요 시 `card-styles.ts`에 변형 추가
- [ ] `ChatBubble` 공통 컴포넌트 추출 — FollowUpQASection, ChatThread에서 메시지 버블 중복
- [ ] `MarkdownCard` 공통 래퍼 검토 — `prose prose-sm dark:prose-invert max-w-none` 4곳 반복

### UX 플로우 개선

- [ ] AI 해석 실패 시 graceful degradation — 2회 retry 후 "일반 해석으로 계속" 대안 제시
- [ ] Step 2 데이터 없이 진입 시 "데이터 먼저 업로드" CTA + Step 1 이동 버튼
- [ ] 변수 자동감지 실패 시 토스트 알림 — `extractDetectedVariables()` try-catch 추가
- [ ] Hub 채팅 에러 시 이전 메시지 유지 — 에러 후 입력창에 원래 텍스트 복원
- [ ] 분석 실행 에러 시 상단 배너 + "재시도" CTA 강화

### 접근성

- [ ] ChatThread, TrackSuggestions 버튼에 focus ring 통일 (`focus-visible:ring-2 focus-visible:ring-primary/40`)
- [ ] ChatThread 업로드 CTA, QuickAnalysisPills raw button → shadcn `<Button>` 전환
...
- [ ] `lib/services/variable-type-detector.ts` — 타입 자동 감지
