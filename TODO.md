...
| # | 항목 | 설명 | 난이도 |
|---|------|------|--------|
| A | **Store 3분할** | analysis-store(26개 상태) → analysisStore + historyStore + modeStore | 중 |
| B | **Method Registry 통합** | 메서드 추가 시 3파일(method-mapping, variable-requirements, SELECTOR_MAP) → 1파일 registerMethod() | 중 |
| C | **Step별 훅 분리** | useAnalysisHandlers(36개 반환) → useDataUploadStep, useMethodSelectStep 등 | 낮음 |
| D | **파생 상태 전환** | methodCompatibility 등 persist 대상에서 제외 → useMemo 자동 파생 | 낮음 |
| E | **트랙 기반 Step 시퀀스** | quickAnalysisMode 등 boolean 플래그 → TRACKS 배열 선언형 전환 | 높음 |
| F | **setValidationResults 부작용 명시화** | 4개 부작용(dataSummary, compatibility, assumption 초기화) → 명시적 파이프라인 | 낮음 |
| G | **consultant-service 카테고리 내 2차 매칭** | "전후 비교" → paired-t 우선 추천 (메서드별 키워드 매칭 추가) | 낮음 |
| H | **두 페이지 스토어 간섭 경고** | Home에서 분석 중 → /analysis 진입 시 기존 분석 덮어쓰기 경고 | 낮음 |

**우선순위**: A → B → C (확장성 기반), G → H (UX 품질)
