...
- [ ] **초안 작성 시작 시 저널 요구사항 주입 설계**: `자료 작성` 시작 단계에서 target journal/profile preset/manual requirements를 선택하고, 같은 profile snapshot을 section blueprint, writer context, review/preflight freshness에 연결한다. 저널 요구사항은 export 직전 사후 점검만이 아니라 초안 생성 prompt/context와 deterministic preflight 기준에 처음부터 반영한다.
