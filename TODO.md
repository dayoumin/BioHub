...
## Graph Studio 차트 라이브러리 평가 결과 (2026-03-20)

**평가 문서**: [CHART_LIBRARY_EVALUATION.md](stats/docs/graph-studio/CHART_LIBRARY_EVALUATION.md)
**결론**: 편집은 ECharts 유지, 논문 제출은 matplotlib export 추가. Plotly.js/Nivo/shadcn은 보류.

### PoC 검증 완료 (2026-03-20)

- [x] P0~P2 모두 성공 — 상세: [CHART_LIBRARY_EVALUATION.md §9](stats/docs/graph-studio/CHART_LIBRARY_EVALUATION.md)

### G5 MVP 완료 (2026-03-20)

- [x] matplotlib export Worker 6 통합 — bar/line/scatter 3종 + SciencePlots 스타일
- [x] ExportDialog "논문용 내보내기" — PDF/TIFF/EPS/SVG + 스타일/DPI 선택
- [x] 2-pass 리뷰 (17건 수정) — race condition, Worker 매핑 누락, rcParams 누출 등

### G5 미완료 (향후)

- [ ] matplotlib 차트 7종 추가 — boxplot, histogram, error-bar, heatmap, violin, km-curve, roc-curve
- [ ] orientation (수평 막대), aggregate (sum/count/min/max/median), facet (다중 패널)
- [ ] significance brackets, annotations (hline/vline), ColorBrewer 6종 팔레트 매핑

### ECharts 대용량 데이터 최적화 (향후)

> 현재 scatter/line은 모든 포인트를 DOM에 렌더. 1만행 이상에서 성능 저하, 10만행+ 브라우저 멈춤 가능.
> matplotlib 논문 출력은 래스터 렌더링이라 포인트 수 영향 적음. 단 Pyodide WASM 메모리 ~2GB 제한 주의.

**데이터 규모별 전략:**

| 행 수 | 시나리오 예시 | 브라우저 상태 | 대응 |
|--------|-------------|-------------|------|
| ~5K | 일반 논문 데이터 | 정상 | 변경 없음 |
| 5K~5만 | 임상 코호트 | 느려질 수 있음 | ECharts `large` 모드 자동 적용 |
| 5만~50만 | 유전체 일부, 대규모 설문 | 렌더링 지연 | 경고 토스트 + 집계/다운샘플 권장 |
| 50만+ | RNA-seq, GWAS | 브라우저 한계 초과 | **차단 + 안내** ("R/Python 데스크탑 권장") |

**구현 단계:**

| 단계 | 내용 | 난이도 |
|------|------|--------|
| **1. 행 수 감지 + 단계별 안내** | 파일 로드 시 행 수 체크 → 구간별 토스트/다이얼로그 자동 표시 | 낮음 |
| **2. large 모드** | echarts-converter에서 데이터 5K+ 시 `large: true` + `largeThreshold` 자동 설정 | 낮음 |
| **3. 다운샘플링** | scatter 1만+ 포인트 → LTTB 알고리즘 자동 다운샘플 옵션 | 중간 |
| **4. ECharts GL** | `scatterGL`/`linesGL` WebGL 렌더러 (100만 포인트) | 중간 |
| **5. 하드 리밋** | 50만행 초과 시 렌더링 차단 + "데스크탑 도구(R/Python) 사용 안내" 다이얼로그 | 낮음 |
| **6. 적응형 성능 안내** | 렌더링 소요 시간 실측 → 3초+ "느림 경고" / 10초+ "취소 옵션" 제공 | 낮음 |

> **적응형 안내 방식**: 행 수 1차 경고 + 렌더링 시간 2차 경고 조합.
> 동일 데이터라도 고사양 PC에서는 경고 안 뜨고, 저사양에서만 뜨는 UX.
> `navigator.deviceMemory`(Chrome만, 대략적) 등 하드웨어 감지는 신뢰도 낮아 실측 기반이 현실적.
>
> **안내 메시지 예시:**
> - 5만행: "⚠️ 데이터가 큽니다 (52,340행). 집계 또는 필터링을 권장합니다."
> - 렌더 3초+: "렌더링이 느립니다. 집계 또는 필터를 적용하면 빨라집니다."
> - 렌더 10초+: "렌더링 시간이 깁니다. 계속 / 취소 / 집계 적용"
> - 50만행+: "🚫 브라우저 메모리 한계를 초과할 수 있습니다 (523,400행). 데이터를 필터링하거나, 대용량 처리에는 R/Python을 권장합니다."

### 별도 프로젝트

- [ ] **ops-dashboard 원격 푸시** — 로컬에만 있음. 푸시 후 모니터링 항목 등록 ([CHART_LIBRARY_EVALUATION.md §10](stats/docs/graph-studio/CHART_LIBRARY_EVALUATION.md) 참조)
...
## 사전 인프라 정리 (개별 작업 전 선행 — 2026-03-20)

상세: [INFRA-PREREQUISITES.md](stats/docs/INFRA-PREREQUISITES.md)

- [x] Focus ring 표준 상수 — `card-styles.ts`에 `focusRing` 추가 (접근성 3건 의존) ✅ 2026-03-21
- [x] 토스트 메시지 상수 — `lib/constants/toast-messages.ts` + use-hub-data-upload 마이그레이션 ✅ 2026-03-21
- [x] `useErrorRecovery` 훅 — retry 카운트 + 소진 판정 + 테스트 8개 ✅ 2026-03-21

---

## 디자인/UX 개선 사항 (2026-03-20 리뷰, 03-21 검증)

**진행 순서** (의존성 기반):
```
[1단계] 데드 코드 제거 + 접근성 (독립, 병렬 가능)
[2단계] ChatBubble 추출 (에러 복구 UI가 의존)
[3단계] UX 플로우 에러 복구 (useErrorRecovery 활용)
[4단계] 반응형 + 인라인 카드 (점진적)
[5단계] 테스트 커버리지 (독립, 언제든 가능)
```

### [1] 데드 코드 제거 + 접근성 (독립)

- [ ] `paper-draft/PaperDraftPanel.tsx` + `PaperDraftPanel.demo.tsx` 삭제 — import 0건, demo 전용. 운영 코드는 `analysis/steps/PaperDraftPanel.tsx`
- [ ] ChatThread, TrackSuggestions 버튼에 `focusRing` 상수 적용 (card-styles.ts에서 import)
- [ ] ChatThread 업로드 CTA → shadcn `<Button variant="outline">` 전환
- [ ] TrackSuggestions 장식용 아이콘에 `aria-hidden="true"` 추가
- [ ] DataContextBadge 토글에 `aria-expanded` 추가 ✅ 완료 (2026-03-21)

### [2] ChatBubble 공통 컴포넌트 추출

- [ ] FollowUpQASection + ChatThread 메시지 버블 → `ChatBubble` 공통 컴포넌트
  - 에러 복구 작업(3단계)에서 retry 버튼을 버블에 추가할 수 있으므로 선행

### [3] UX 플로우 에러 복구 (`useErrorRecovery` 활용)

- [ ] AI 해석 실패 시 graceful degradation — `useErrorRecovery(2)` + 소진 시 "일반 해석으로 계속" 대안
- [ ] 변수 자동감지 빈 결과 시 토스트 — `extractDetectedVariables()` 반환값 검증 (throw하지 않음, 빈 결과 반환). 컴포넌트이므로 `t.xxx` 사용
- [ ] Hub 채팅 에러 시 이전 메시지 유지 — 에러 후 입력창에 원래 텍스트 복원
- [ ] 분석 실행 에러 시 상단 배너 + "재시도" CTA 강화

### [4] 반응형 + 인라인 카드 (점진적)

- [ ] 그리드 breakpoint 패턴 통일 — TrackSuggestions(`md:grid-cols-4`) vs ChatThread(`sm:grid-cols-2`)
- [ ] 반응형 텍스트 사이징 — 주요 텍스트에 `lg:` 변형 추가
- [ ] 인라인 카드 패턴 — `p-4 rounded-xl border` 10+곳 반복. 필요 시 `card-styles.ts`에 변형 추가 (점진적)

### [5] 테스트 커버리지 (독립, 언제든 가능)

- [ ] `hooks/use-analysis-handlers.ts` — CRITICAL: 17개 리턴값, 분석 플로우 핵심
- [ ] `lib/services/data-validation-service.ts` — 데이터 검증 로직
- [ ] `lib/stores/template-store.ts` — IndexedDB 경쟁 조건
- [ ] `hooks/use-statistics-page.ts` — 레거시 45개 페이지 의존

### 해결됨 / 제거

- [x] `MarkdownCard` 공통 래퍼 → `proseBase` 상수로 해결 (card-styles.ts). 별도 컴포넌트 불필요
- [x] `variable-type-detector.ts` 테스트 → `variable-detection-service.test.ts`에 45개 테스트 이미 존재
- [x] Step 2 데이터 없이 진입 CTA → AnalysisLayout step guard가 이미 차단. 발생 불가
