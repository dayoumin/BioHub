...
> BLAST 오케스트레이션 리팩토링 (BlastRunner + GenericBlastRunner 통합): [PLAN-BLAST-RUNNER-UNIFICATION.md](../stats/docs/bio-tools/PLAN-BLAST-RUNNER-UNIFICATION.md)
...
- [x] `pnpm test` 통과 (81/81 genetics 테스트)
- [x] Phase 0: 각 버그 재현 후 수정 확인
- [x] A1: 다중 서열 입력 → GC%/염기 조성/길이 분포 차트 → CSV 내보내기 → 히스토리 저장/복원
- [x] A2: 정렬된 FASTA → 거리 행렬 히트맵 + 덴드로그램 → CSV 내보내기
- [x] A3: FASTA → NJ/UPGMA 트리 시각화 → Newick 내보내기
- [x] B1: 바코딩 결과에서 CSV 다운로드
- [x] B2: GenBank → BLAST/Barcoding 서열 자동 입력 확인
- [x] B3: 히스토리 사이드바에서 종명 검색 → 필터 동작
- [x] 랜딩 페이지: 6개 도구 모두 "사용 가능" 표시

## 구현 완료 (2026-04-04)

모든 Phase 0 + Phase A + Phase B 항목 구현 완료.
추가 리팩토링: worker.ts 분리 (1,425→65줄), docs 구조 정리 (89→13 서브폴더).
알려진 제한: ECharts tree series는 토폴로지만 표시 (거리 비례 미지원) — UI에 명시.
