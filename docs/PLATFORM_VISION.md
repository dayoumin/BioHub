...
BioHub는 논문이나 보고서를 한 번에 완성하는 도구가 아니라, 연구자가 만든 산출물을 부분별로 구조화하고 검증한 뒤 문서 단위로 통합하는 반자동 작성 플랫폼이다.

- 부분별 작성: Analysis, Graph Studio, Bio-Tools, 유전적 분석, Literature 결과를 각각 독립적인 연구 산출물로 정리한다.
- 체계적 통합: 각 산출물을 provenance, source binding, document section 단위로 연결해 하나의 문서로 조립한다.
- 보수적 자동화: 검증된 값과 사용자가 확인한 입력만 자동 문장화하고, 시스템이 모르는 내용은 추론하지 않는다.

핵심 방향은 `부분별 작성 -> 체계적 통합 -> 보수적 자동화`다.

### 2-2. 보수적 자동화

BioHub의 AI/자동화 기능은 `많이 써주는 AI`보다 `틀린 말을 하지 않는 연구 보조 시스템`을 우선한다.

- 검증된 분석 결과와 사용자가 확인한 입력만 자동 산출물에 사용한다.
- 연구 설계, 결측 처리, 전처리, 제외 기준, 분석 선택 이유처럼 시스템이 알 수 없는 내용은 추론하지 않는다.
- 근거가 부족한 항목은 자동 문장으로 만들지 않고 `보완 필요` 또는 `검토 필요`로 표시한다.
- 자동화 산출물은 최종 결론이 아니라 연구자가 검토하고 반영하는 초안이다.
- 자동화 범위는 provenance, 재생성 가능성, 사용자 책임 경계가 확보된 뒤 단계적으로 넓힌다.

이 원칙은 논문 Methods/Results 작성뿐 아니라 Graph Studio, Bio-Tools, Literature 등 모든 연구 보조 기능에 적용한다.
