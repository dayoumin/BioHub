...
> **수치 정밀도 정책**: export 시 raw number를 그대로 직렬화하면 AI가 "정확히 그대로" 규칙을 지키면서도 저널 스타일(`p = .017` 등)과 충돌할 수 있다.
> BioHub 분석 엔진이 이미 유효자리수를 처리한 값을 출력한다면 충돌이 없다. 처리 전 raw 값(예: `0.017234`)이 저장되는 경우, 향후 `displayValue: string` 필드(예: `".017"`)를 추가하여 AI에는 displayValue를 전달한다. **현재는 분석 엔진 출력 정밀도에 의존하고, 문제 발생 시 displayValue 필드 추가로 해결.**
...
패턴 요약 생성 전략:
- **자동 (1순위)**: `GraphProject.analysisId` → `HistoryRecord`의 분석 결과(기술통계)에서 추출. DataPackage(원본 CSV) 재로드 불필요.
- **수동 fallback (2순위)**: HistoryRecord 연결이 없거나 추출 불가 시 UI에서 textarea로 직접 입력.
- **Phase 16 이후**: DataPackage를 Cloudflare R2에 저장하면 완전 자동 생성으로 업그레이드 예정.

이미지 없이도 AI가 Figure를 참조하는 문장 작성 가능.
...
> **모델 역할 분리**
> - `PaperPackage` / `assemblePaperPackage()` — **AI 프롬프트 빌더**: 분석 결과를 Markdown + JSON으로 직렬화하여 외부 AI(Claude/GPT/Gemini)에 붙여넣는 export 산출물.
> - `DocumentBlueprint` / `document-assembler.ts` — **내부 WYSIWYG 편집 모델**: Plate 에디터용 섹션 기반 구조. BioHub 안에서 편집하고 저장하는 문서.
>
> 두 모델은 목적이 달라 병렬 존재가 의도된 설계다. 혼동 금지.
...
interface PackageItem {
  ...
  analysisIds: string[];              // 분석 간 cross-reference용 ("ANAL-01"). 그림/표는 여러 분석을 동시 참조 가능하므로 배열.
  label: string;                      // "Table 1", "Figure 2"
  section: 'results' | 'methods' | 'discussion';
  order: number;
  included: boolean;                  // 체크박스로 제외 가능
  /** 그래프 패턴 요약. HistoryRecord 기반 자동 생성 시도 → 불가 시 수동 입력. */
...
interface PackageReference {
  ...
  /**
   * 요약 작성 상태. included=true 인 ref는 export 전에 'ready'여야 한다.
   * - 'missing': 요약 없음 (UI에서 경고 표시)
   * - 'draft': AI 자동 제안 상태 (사용자 확인 필요)
   * - 'ready': 사용자 확인 완료
   */
  summaryStatus: 'missing' | 'draft' | 'ready';
  included: boolean;
}
...
function assemblePaperPackage(pkg: PaperPackage): AssemblyResult {
  ...
  //    섹션 순서는 JournalPreset.sections 배열을 따름 (하드코딩 아님)
  //    커스텀 저널: sections ID와 섹션별 지침 매핑 규약은 구현 시 정의
  // 3. 섹션별 지침 + 통계 보고 예시 (Markdown)
  // 4. 연구 개요 (Markdown)
  // 5. 분석 결과 (JSON 블록)
  // 6. 그래프 (Markdown — PackageItem.patternSummary 사용)
  // 7. 참고문헌 (Markdown — summaryStatus='ready'인 항목만 포함, 'missing'/'draft'이면 경고)
  // 8. 추가 맥락 (Markdown)
...
// 그래프 패턴 요약 생성
// - graph.analysisId → HistoryRecord 기술통계에서 자동 추출 (DataPackage 불필요)
// - HistoryRecord 연결 없으면 undefined 반환 → UI에서 수동 입력 fallback
// Phase 16: DataPackage R2 저장 후 graph + dataPackage 직접 접근으로 업그레이드
function generateFigurePatternSummary(
  graph: GraphProject,
