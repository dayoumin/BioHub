...
- **TODO.md**: 현황 + 할일
- **TODO.md는 공유 작업 목록**: 사용자가 직접 추가/수정/삭제하며 여러 환경에서 pull/push로 이어서 사용하는 기준 문서로 취급한다.
- **TODO.md 변경 기본 원칙**: 의도된 할일 업데이트는 기본적으로 커밋 대상이다. 단, 인코딩 깨짐·줄바꿈 churn처럼 비의도 변경이 의심되면 실제 UTF-8 내용을 먼저 확인한 뒤 처리한다.
- **ROADMAP.md**: 개발 로드맵
- **ROADMAP.md 용도**: AI 코딩으로 1주 이상 걸릴 수 있는 작업, 여러 단계의 장기 과제, 당장 착수하지 않을 중장기 계획을 두는 문서로 취급한다.
