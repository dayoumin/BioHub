...
**목표**: StatisticsTable 적용 확대 + 내보내기 버튼 비활성화

#### 1. manova 페이지 완료 (6개 테이블, 60분)

**적용 테이블**:
1. Table 1: 다변량 검정 통계량 (Lines 588-640)
2. Table 2: 개체 간 효과 검정 (Lines 652-710)
3. Table 3: 정준 판별 분석 (Lines 744-782)
4. Table 4: 정준 상관 분석 (Lines 794-827)
5. Table 5: 표준화 판별계수 (Lines 839-873)
6. **Table 6 스킵**: 그룹 중심값 (Lines 885-904) - 각 판별함수별 개별 테이블, 중첩 구조로 부적합
7. Table 7: 분류 결과 (Lines 949-1009)

**수정 파일**:
- app/(dashboard)/statistics/manova/page.tsx (Lines 588-1062)

**코드 감소**: 362줄 → 157줄 (-23%)

**커밋**:
- `10082bb` - feat(phase3): manova 2개 테이블 StatisticsTable 변환 (중간 커밋)
- `f312c3c` - feat(phase3): manova 6/6 변환 가능 테이블 완료

#### 2. 내보내기 버튼 비활성화 (14개 페이지, 45분)

**작업 내용**:
- 비활성화된 내보내기 버튼에 Tooltip 추가 ("향후 제공 예정입니다")
- UITooltip import 자동 추가 (recharts Tooltip과 충돌 방지)

**스크립트**: scripts/disable-export-buttons.py (185 lines)

**적용 페이지** (14개):
- anova, cochran-q, correlation, discriminant
- ks-test, manova, mcnemar, mixed-model
- mood-median, pca, regression, runs-test
- sign-test, t-test

**포맷팅 수정**: scripts/fix-tooltip-formatting.py (91 lines)
- 5개 페이지 빈 줄 제거: mcnemar, ks-test, regression, runs-test, sign-test

**총 적용**: 22개 페이지 (이전 8개 + 이번 14개)

**커밋**:
- `a0366f3` - feat(phase3): 14개 페이지 내보내기 버튼 비활성화 및 Tooltip 추가
- `2bd250a` - fix(phase3): 3개 페이지 Tooltip 포맷팅 수정

#### 3. mann-whitney + reliability 변환 (3개 테이블, 25분)

**mann-whitney/page.tsx** (2개 테이블):
1. U 검정 통계량 (Lines 456-495)
   - PValueBadge를 custom formatter로 렌더링
2. 집단별 기술통계량 (Lines 507-548)
   - 범위 필드: 문자열 보간으로 처리

**reliability/page.tsx** (1개 테이블):
- 항목별 통계량 (Lines 533-560)
- 조건부 span 스타일링 (alphaIfDeleted 비교)

**스크립트**:
- scripts/convert-mann-whitney-tables.py (170 lines)
- scripts/convert-reliability-table.py (80 lines)

**TypeScript 수정**:
- mann-whitney: StatisticsTable import 누락 추가
- mann-whitney: `as const` 타입 추론 수정

**커밋**: `c422b18` - feat(phase3): mann-whitney, reliability 테이블 변환

#### 4. partial-correlation 변환 (2개 테이블, 30분)

**적용 테이블**:
1. 편상관계수 테이블 (Lines 468-509)
   - getCorrelationStrength 함수 활용
   - 동적 Badge 스타일링 (강도: 강함/중간/약함/매우 약함)
   - p-value < 0.05 시 * 표시

2. 편상관 vs 단순상관 비교 (Lines 524-576)
   - 인라인 조건부 로직 (absChange > 0.2/0.1/0.05)
   - 차이 필드: + 접두사, 동적 색상
   - 해석 필드: Badge 컴포넌트

**스크립트**: scripts/convert-partial-correlation-tables.py (210 lines)

**TypeScript**: 0 errors ✓

**커밋**: `e47dc58` - feat(phase3): partial-correlation 2개 테이블 변환

---

### 📊 Phase 3 누적 성과 (2025-11-12 기준)
...
**스킵된 복잡한 페이지** (4개):
1. **chi-square-goodness** - 진행바(progress bar), tfoot, 복잡한 시각화
2. **chi-square-independence** - 동적 2×k 분할표 (런타임 행/열 개수)
3. **mood-median** - 2×k 동적 컬럼 구조
4. **mcnemar** - 2×2 테이블 with colSpan/rowSpan

---

### 📋 복잡한 페이지 처리 계획

#### 옵션 1: StatisticsTable 확장 (고비용)
...
**예상 시간**: 각 기능 2-3시간 (총 6-9시간)

**장점**: 완전한 일관성
**단점**: 컴포넌트 복잡도 증가, 과도한 추상화 위험

#### 옵션 2: 특수 컴포넌트 개발 (중비용)

**새 컴포넌트**:
- `ContingencyTable` (분할표 전용)
- `ProgressTable` (진행바 지원)
- `MatrixTable` (colSpan/rowSpan)

**예상 시간**: 각 1-2시간 (총 3-6시간)

**장점**: 목적별 최적화, StatisticsTable 단순 유지
**단점**: 컴포넌트 수 증가

#### 옵션 3: 현재 상태 유지 (권장, 저비용)

**근거**:
- 4개 페이지는 특수 케이스 (전체 45개 중 9%)
- 현재 구현도 충분히 명확함
- 복잡도 대비 ROI 낮음

**예상 시간**: 0시간

**결정**: ✅ **옵션 3 권장** - 핵심 19개 테이블 변환 완료로 목표 달성

---

### 📅 다음 할일 (2025-11-13)

#### 옵션 A: Phase 3 마무리 및 문서화 ⭐ (권장)

**작업**:
1. STATUS.md Phase 3 섹션 업데이트
2. ROADMAP.md Phase 3 완료 표시
3. Phase 3 최종 요약 보고서 (선택)
...
**대상**:
- EffectSizeCard (2-3개 페이지)
- StatisticalResultCard (2-3개 페이지)
