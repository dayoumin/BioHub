...
**포지션**: SPSS급 통계 + PAST급 생태지수 + rfishbase급 DB연동 → 코딩 없이, 브라우저에서, 무료로

**AI 시대 차별점**: 범용 AI(ChatGPT/Claude)가 코드를 생성해줘도, BioHub는 (1) 검증된 라이브러리 사용 = hallucination 없음 (2) 재현성 보장 (3) 데이터 프라이버시 (브라우저 내 실행) (4) 워크플로우 연결 (분석→논문→문헌이 한 곳에서)로 차별화.

**데이터 보안 차별점** (개인 + 기관):
- **개인 연구자**: 데이터가 내 컴퓨터를 절대 떠나지 않음 (Pyodide 브라우저 실행 + Tauri 데스크탑). 클라우드 업로드 없이 분석 가능 → 미공개 연구 데이터 보호
- **기관/연구소**: 내부 서버 배포로 기관 밖 데이터 유출 원천 차단. IRB/개인정보보호법 준수 용이. 기관 자체 인증(SSO) 연동 가능
- SPSS/R은 클라우드 라이선스 확산 중, BioHub는 반대로 **완전 로컬 실행**을 핵심 가치로 유지

**락인 효과** (사용할수록 전환 비용 증가):
- 분석 히스토리 축적 (재현 가능, 타 도구 이관 불가)
- 커스텀 템플릿/분석 레시피 (개인화 데이터)
- 종 정보 즐겨찾기 + 인용 라이브러리
- 단, 데이터 내보내기(export)는 항상 지원 → 신뢰 = 더 오래 머무름

**해자 전략**:
- 단기(1년): 도메인 특화 (수산과학 용어/기준/예시) → 범용 도구가 쉽게 못 따라옴
- 중기(2~3년): 사용자 데이터 축적 + 커뮤니티 콘텐츠 (분석 레시피, 해석 가이드)
- 장기(3년+): 네트워크 효과 (공유 템플릿, 크라우드소싱 DB) + 논문 인용 ("BioHub를 사용하여 분석하였다")

**초기 전략**: 특정 학회(한국수산과학회 등) 워크숍/튜토리얼 개최 → 연구실 단위 채택 → 학회 내 표준화
...
- 개발이 어느 정도 진행된 뒤 자료 작성 제품 기준 점검
  - 진입 시간
  - 작성 진행 가시성
  - 수동 편집 보존
...
연구 데이터가 사용자/기관의 통제 하에 머물도록 다양한 배포 모드를 지원한다.

Includes:

- **개인 로컬 모드**: Tauri 데스크탑 앱 — 데이터가 내 컴퓨터에서만 처리, 외부 전송 없음
- **기관 내부 서버 모드** (2027 H2): 연구소/대학 내부 서버에 자체 배포 — 기관 방화벽 안에서 운영
  - Docker/컨테이너 기반 배포 패키지
  - 기관 인증(SSO/LDAP) 연동
  - 다중 사용자 동시 접속 + 공유 프로젝트
  - 기관 데이터 정책 준수 (IRB, 개인정보보호법)
- 웹 버전은 기존 Cloudflare Pages 유지 (Pyodide 브라우저 실행으로 데이터 미전송)
...
### Horizon 3. Research Copilot + 개인 로컬 모드
...
- Stream 5. Research Copilot
- Stream 6. Data Sovereignty — 개인 로컬 모드 (Tauri 데스크탑)
...
- Tauri 데스크탑 앱 — 완전 오프라인, 데이터 로컬 보관

### Horizon 4. 기관 내부 서버 배포 (2027 H2)

Build after Horizon 3 — 개인 사용자 기반 확보 후.

Priority streams:

- Stream 6. Data Sovereignty — 기관 서버 모드

Must deliver:

- Docker 기반 기관 내부 배포 패키지
- 다중 사용자 + SSO/LDAP 인증
- 기관 데이터 정책 준수 가이드
- 관리자 대시보드 (사용량, 사용자 관리)
...
### Phase G. 데이터 보안 & 배포 모드

**G-1. 개인 로컬 모드 (Tauri 데스크탑)**

Scope:

- Tauri 데스크탑 앱으로 완전 오프라인 분석 환경
- 연구 데이터가 사용자 PC를 벗어나지 않음 (네트워크 차단 상태에서도 동작)
- 로컬 파일 시스템 직접 접근 (대용량 데이터)
- 자동 업데이트 (온라인 시)

Exit criteria:

- 인터넷 없이 모든 핵심 분석 기능 동작
- 데이터가 외부로 전송되지 않음을 검증 가능 (네트워크 모니터링 투명성)

**G-2. 기관 내부 서버 모드 (2027 H2)**

Scope:

- Docker/컨테이너 기반 온프레미스 배포 패키지
- 다중 사용자 동시 접속 + 공유 프로젝트
- 기관 인증 연동 (SSO/LDAP/Active Directory)
- 관리자 대시보드 (사용자 관리, 사용량 통계)
- 기관 데이터 정책 준수 가이드 (IRB, 개인정보보호법)

Exit criteria:

- 기관 방화벽 내에서 외부 접속 없이 전체 기능 운영 가능
- 관리자가 사용자/프로젝트를 관리할 수 있음
- 기관 보안 감사 체크리스트 통과

Dependencies:

- Phase A (프로젝트 모델) + Phase 16 (Cloudflare Workers 백엔드 → 서버 모드로 전환 가능한 아키텍처)
...
9. **개인 로컬 모드 G-1: Tauri 데스크탑 (데이터 보안 — 개인 연구자)**
10. **기관 내부 서버 G-2: 온프레미스 배포 (데이터 보안 — 기관/연구소, 2027 H2)**
...
### 모바일 대응 전략 (후순위 — 주요 기능 안정화 이후)

**현재**: 통계 분석·그래프 편집·논문 작성 등은 **PC 전용**. 모바일 접속 시 별도 안내 없음.

**원칙 (2026-03-30 결정)**:
- **핵심 기능 (분석, 그래프, 논문 편집)**: PC 웹 전용. 모바일 레이아웃 고려 안 함
- **모바일 역할**: 알림 확인, 결과 조회, 상태 확인 등 읽기 전용
- **향후 필요 시**: 모바일 전용 경량 뷰 별도 검토 (편집 기능 아닌 뷰어/알림)

**구현 목표**:
- Phone (< 768px): "PC에서 접속해 주세요" 전체 화면 안내 + URL 복사 버튼
- Tablet (768~1024px): 배너 경고 + 제한적 탐색 허용
- 표본 크기 계산기 등 단순 도구는 모바일 예외 처리 검토

**구현 방식**: `app/layout.tsx`에 `MobileGuard` 컴포넌트 — 모든 페이지 일괄 적용
...
**최종 업데이트**: 2026-03-30
**현재 Phase**: 논문 작성 기능 Phase 1 구현 시작
**아키텍처 결정**: Smart Flow = 통계 진입점, Bio-Tools = 별도 섹션, /statistics/* = 레거시, 논문 작성 = PC 전용
**다음 마일스톤**: 논문 작성 Phase 1 (DocumentBlueprint 기반)

**최근 완료**:
- Bio-Tools 검증 보고서 작성 (12개 확정, 6개 제외, Pyodide 호환성 검증) (2026-02-13)
- Graph Studio ECharts 전환 + 유틸/스토어 보강 + 102개 테스트 (2026-02-28)
- 논문 작성 지원 기능 6단계 구현 계획 승인 (2026-03-30)
...
## 📋 백로그: terminology 분리 기준 재검토 (보류)

**목표**: UI 문자열을 `terminology`로 옮길 기준을 정리한다.

**현재 판단**:
- 통계 용어, 단계명, 결과 해석, 경고 문구처럼 도메인/언어/재사용 영향이 큰 텍스트는 `terminology` 우선
- 한 컴포넌트 안에서만 쓰이는 구조적 보조 문구는 로컬 유지 가능
- 허브/온보딩/빈 상태 카피는 양이 커지거나 도메인별 차이가 생길 때 묶어서 이동 검토

**검토 대상 예시**:
- `TrackSuggestions` 같은 허브 entry-point 설명 문구
- empty state / helper copy / quick action 설명 문구

**주의사항**:
- `terminology = 모든 UI 텍스트 저장소`로 가면 과도해질 수 있음
- 번역/도메인 차이/재사용성이 명확할 때만 이동하는 원칙 유지
...
| 시기 | 항목 | 설명 |
|------|------|------|
| 2026-09 | `@echarts-x/custom-violin` 재평가 | KDE 범위 하드코딩 버그(#12, #21) 해결 여부 확인. 현재 자체 KDE+renderItem 구현 사용 중. 해결되어도 전환 실익 낮음 — 참고만. |
