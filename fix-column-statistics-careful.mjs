...
// ColumnStatistics 객체에 numericCount와 textCount 추가 (신중하게)
function fixColumnStatistics(content) {
  let modified = content;

  // Pattern 1: type: 'numeric' 이면서 numericCount가 없는 경우만 추가
  ...
  // 더 정교한 정규식: type: 'numeric' 다음에 uniqueValues가 바로 오는 경우만
  modified = modified.replace(
    /(\btype:\s*['"]numeric['"],)\s*(\bunique)/g,
    "$1\n      numericCount: 100,\n      textCount: 0,\n      $2"
  );

  // Pattern 2: type: 'categorical' 다음에 uniqueValues가 바로 오는 경우만
  modified = modified.replace(
    /(\btype:\s*['"]categorical['"],)\s*(\bunique)/g,
    "$1\n      numericCount: 0,\n      textCount: 100,\n      $2"
  );
